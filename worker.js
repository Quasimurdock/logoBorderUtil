...
onmessage = async function (ev) {
    ...
    await Jimp.read(inputPath + filename, async function (err, img) {
        ...
        const bottomBorderHeight =
            initialBottomBorderHeight > 298 ? initialBottomBorderHeight : 298;
        ...
        await Jimp.loadFont(Jimp.FONT_SANS_64_BLACK).then(async (font) => {
            ...
            const FocalLengthString =
                FocalLength == null || typeof FocalLength === "undefined"
                    ? ""
                    : `${FocalLength}mm  `;
            const FNumberString =
                FNumber == null || typeof FNumber === "undefined"
                    ? ""
                    : `F${FNumber}  `;
            const ISOString =
                ISO == null || typeof ISO === "undefined" ? "" : `ISO${ISO}  `;
            const ShutterString =
                Shutter == null || typeof Shutter === "undefined"
                    ? ""
                    : `1/${Shutter}s  `;
            //拼接EXIF基本信息字符串
            ...
            borderedImg.print(
                font,
                img.bitmap.width * 0.02,
                img.bitmap.height + bottomBorderHeight * 0.7,
                basicExifInfoString
            );
            //照片日期打到边框上
